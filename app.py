...
def latestvideos():
    ...
    for sourceID in datadict:
        ...
        if not newvideos:
            marked.append(sourceID)
        else:
            datadict[sourceID]["entries"] = newvideos
    for sourceID in marked:
        del datadict[sourceID]
    return datadict
...
def index():
    return render_template(
        "index.html", sources=dataobj.sourceList(), datadict=latestvideos()
    )
